...
import {
  ...
  X, Shield, Target, Activity, Clock
} from 'lucide-react'
import {
...
// Nombres descriptivos para cada ticker
const TICKER_NAMES = {
...
const fmt  = (n, d = 2) => n == null || n !== n ? '–' : n >= 1000 ? n.toLocaleString('es-MX', {minimumFractionDigits:2,maximumFractionDigits:2}) : Number(n).toFixed(d)
...
function SignalBadge({ signal, size = 'sm' }) {
  ...
  return (
    <span style={{
      ...
      padding: pad, borderRadius: 5, fontSize: fs, fontWeight: 700,
      whiteSpace: 'nowrap', boxShadow: `0 0 8px ${c.glow}`
    }}>
      {c.label}
    </span>
  )
...
function MiniSparkline({ data }) {
  ...
  const color = data.at(-1)?.v >= data[0]?.v ? '#4ade80' : '#f87171'
  return (
...
function Card({ children, style }) {
  return (
    <div style={{
      ...
      borderRadius: 8, padding: 12, ...style
    }}>
      {children}
    </div>
  )
...
function SectionTitle({ children }) {
  return (
    <div style={{ fontSize: 10, color: '#475569', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>
      {children}
    </div>
  )
}
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.75)', zIndex:200,
      ...
      <div style={{ background:'#0f172a', border:'1px solid #334155', borderRadius:10,
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          <h3 style={{ color:'#f1f5f9', fontWeight:700 }}>{title}</h3>
...
function Toasts({ toasts, onDismiss }) {
  return (
    <div style={{ position:'fixed', bottom:24, right:24, zIndex:300, display:'flex', flexDirection:'column-reverse', gap:10, maxWidth:360 }}>
      {toasts.map(t => {
        ...
        return (
          <div key={t.id} style={{
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
              ...
              <button onClick={()=>onDismiss(t.id)} style={{ background:'none', border:'none', color:'#64748b', cursor:'pointer' }}><X size={14}/></button>
            </div>
            {t.reasons?.length > 0 && (
...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      s.forEach(x => { sigMap[x.ticker] = x; priceMap[x.ticker] = { price: x.price, change_pct: x.change_pct } })
      setSignals(sigMap)
  ...
  useEffect(() => {
    loadAll(); loadHistory()
    // Refresh signals every 60s to catch newly computed indicators
    const t = setInterval(loadAll, 60_000)
    return () => clearInterval(t)
  }, [loadAll, loadHistory])

  // Load real price history + fresh signal when selected ticker changes
  useEffect(() => {
    if (!selected) return
    ...
    Promise.all([
      ...
      if (sig?.ticker) {
        setSignals(prev => ({ ...prev, [selected]: sig }))
      }
    ...
    if (msg.type === 'prices') {
      setPrices(prev => {
        ...
        msg.data?.forEach(({ ticker, price, change_pct }) => {
          ...
          setPriceHist(ph => ({ ...ph, [ticker]: [...(ph[ticker] || []), { v: price }].slice(-40) }))
        })
        return next
      })
    ...
    if (msg.type === 'signal_alert') {
      ...
      if (!notified.current.has(key)) {
        ...
        setSignals(prev => {
          ...
          return { ...prev, [msg.ticker]: { ...existing, signal: msg.signal } }
        })
      }
    }
  ...
  // ── render ───────────────────────────────────────────────────────────────────
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'#080d18', color:'#e2e8f0', overflow:'hidden' }}>

      {/* ── TOP BAR ── */}
      <header style={{ background:'#0a0f1e', borderBottom:'1px solid #1e293b',
        padding:'0 16px', height:52, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:20, alignItems:'center' }}>
          ...
          <div style={{ fontSize:10, color:'#334155' }}>
            {lastUpdate ? lastUpdate.toLocaleTimeString('es-MX') : 'Cargando...'}
          </div>
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
        ...
        <aside style={{ width:252, background:'#0a0f1e', borderRight:'1px solid #1e293b',
          ...
          <div style={{ display:'flex', borderBottom:'1px solid #1e293b' }}>
            {[
              { id:'watchlist', icon:<List size={13}/>,     label:'Lista' },
              { id:'portfolio', icon:<Briefcase size={13}/>,label:'Portafolio' },
              { id:'alerts',    icon:<Bell size={13}/>,      label:`Alertas${activeAlerts.length ? ` (${activeAlerts.length})` : ''}` },
            ].map(t => (
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'6px 0' }}>
            ...
            {leftTab === 'watchlist' && Object.entries(WATCHLIST).map(([group, tickers]) => {
              const SIG_ORDER = { BUY: 0, SELL: 1, HOLD: 2, undefined: 3 }
              const sorted = [...tickers].sort((a, b) =>
                (SIG_ORDER[signals[a]?.signal] ?? 3) - (SIG_ORDER[signals[b]?.signal] ?? 3)
              )
              return (
              <div key={group}>
                <div style={{ fontSize:10, color:'#334155', padding:'6px 10px 2px', fontWeight:700, textTransform:'uppercase', letterSpacing:1 }}>
                  {group}
                </div>
                {sorted.map(ticker => {
                    ...
                    <div key={ticker} onClick={() => setSelected(ticker)} style={{
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <div>
                          ...
                          {TICKER_NAMES[ticker] && <span style={{ fontSize:10, color:'#475569', marginLeft:5 }}>{TICKER_NAMES[ticker]}</span>}
                        </div>
            ...
            {leftTab === 'portfolio' && (
              <div style={{ padding:8 }}>
                ...
                {portfolio?.positions?.map(pos => (
                  <div key={pos.id} style={{ background:'#13192e', borderRadius:6, padding:9, marginBottom:6, border:'1px solid #1e293b' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    ...
                    {pos.stop_loss > 0 && (
                      <div style={{ fontSize:10, color:'#f87171', marginTop:2 }}>
                        🛑 Stop: {fmtU(pos.stop_loss)}
                      </div>
                    )}
                    {pos.take_profit > 0 && (
                      <div style={{ fontSize:10, color:'#4ade80' }}>
                        🎯 Objetivo: {fmtU(pos.take_profit)}
                      </div>
                    )}
                  </div>
                ))}
            ...
            {leftTab === 'alerts' && (
              <div style={{ padding:8 }}>
                ...
                {(() => {
                  ...
                  return sigAlerts.length > 0 && (
                    <div style={{ marginBottom:10 }}>
                      <div style={{ fontSize:10, color:'#475569', fontWeight:700, letterSpacing:1, textTransform:'uppercase', marginBottom:5 }}>
                        🤖 Señales automáticas
                      </div>
                      {sigAlerts.map(a => {
                        ...
                        return (
                          <div key={a.id} style={{
                            ...
                            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                              ...
                              <button onClick={() => { deleteAlert(a.id).then(() => fetchAlerts().then(({alerts:al})=>setAlerts(al))) }}
                            ...
                            <div style={{ fontSize:10, color:'#475569', marginTop:3 }}>
                              {new Date(a.created_at+'Z').toLocaleString('es-MX',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}
                            </div>
                          </div>
                        )
                      })}
                ...
                <button onClick={() => setShowAddAlert(true)} style={{
                  ...
                  <Plus size={13}/> Agregar alerta de precio
                </button>
                {alerts.filter(a => a.alert_type === 'price').length === 0 && (
                  <p style={{ fontSize:11, color:'#334155', textAlign:'center' }}>Sin alertas de precio</p>
                )}
        ...
        {/* ── CENTER PANEL ── */}
        <main style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', padding:12, gap:10, minWidth:0 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              {selPrice && (
                <>
                  ...
                  <span style={{ fontSize:13, fontWeight:600, color:pctC(selPrice.change_pct), display:'flex', alignItems:'center', gap:3 }}>
            ...
            {(loading || chartLoading) ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top:8, right:12, left:8, bottom:4 }}>
                  ...
                  {ind.bb_upper > 0 && <ReferenceLine y={ind.bb_upper} stroke="#ca8a04" strokeDasharray="4 2" label={{ value:'BB+', fill:'#ca8a04', fontSize:9 }}/>}
                  {ind.bb_lower > 0 && <ReferenceLine y={ind.bb_lower} stroke="#ca8a04" strokeDasharray="4 2" label={{ value:'BB-', fill:'#ca8a04', fontSize:9 }}/>}
                  {ind.ema200   > 0 && <ReferenceLine y={ind.ema200}   stroke="#a78bfa" strokeDasharray="3 3" label={{ value:'EMA200', fill:'#a78bfa', fontSize:9 }}/>}
                  {ind.ema50    > 0 && <ReferenceLine y={ind.ema50}    stroke="#fb923c" strokeDasharray="3 3" label={{ value:'EMA50', fill:'#fb923c', fontSize:9 }}/>}
                  {selSig?.stop_loss_buy  > 0 && selSig.signal === 'BUY'  &&
                    <ReferenceLine y={selSig.stop_loss_buy}  stroke="#ef4444" strokeDasharray="5 3" label={{ value:'STOP', fill:'#ef4444', fontSize:9 }}/>}
                  {selSig?.take_profit_buy > 0 && selSig.signal === 'BUY' &&
                    <ReferenceLine y={selSig.take_profit_buy} stroke="#4ade80" strokeDasharray="5 3" label={{ value:'OBJETIVO', fill:'#4ade80', fontSize:9 }}/>}
        ...
        {/* ── RIGHT PANEL ── */}
        <aside style={{ width:230, background:'#0a0f1e', borderLeft:'1px solid #1e293b',
              ...
              <>
                <div>
                  ...
                  <div style={{ display:'flex', gap:6, marginTop:8 }}>
                    <StatBox label="Condiciones ↑" value={`${selSig.buy_count}/5`}  color="#4ade80"/>
                    <StatBox label="Condiciones ↓" value={`${selSig.sell_count}/5`} color="#f87171"/>
                  </div>
                </div>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                      <div style={{ background:'#1a0a0a', border:'1px solid #7f1d1d', borderRadius:6, padding:'8px 10px' }}>
                        ...
                        <div style={{ fontSize:9, color:'#64748b' }}>
                          −{fmtU(selPrice.price - selSig.stop_loss_buy)} ({fmt((selPrice.price - selSig.stop_loss_buy) / selPrice.price * 100)}%)
                        </div>
                      </div>
                      <div style={{ background:'#0a1a0a', border:'1px solid #14532d', borderRadius:6, padding:'8px 10px' }}>
                        ...
                        <div style={{ fontSize:9, color:'#64748b' }}>
                          +{fmtU(selSig.take_profit_buy - selPrice.price)} ({fmt((selSig.take_profit_buy - selPrice.price) / selPrice.price * 100)}%)
                        </div>
                      </div>
                      <div style={{ background:'#13192e', border:'1px solid #1e293b', borderRadius:6, padding:'7px 10px' }}>
                        ...
                        <div style={{ fontSize:9, color:'#475569', marginTop:3 }}>
                          ATR: {fmtU(selSig.atr)} (volatilidad)
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {/* Active conditions */}
                {(selSig.active_buy_reasons?.length > 0 || selSig.active_sell_reasons?.length > 0) && (
                  <div>
                    <SectionTitle>Condiciones Activas</SectionTitle>
                    {selSig.active_buy_reasons?.map((r, i) => (
                      <div key={i} style={{ fontSize:11, color:'#4ade80', marginBottom:4, display:'flex', gap:5 }}>
                        <span>✓</span><span>{r}</span>
                      </div>
                    ))}
                    {selSig.active_sell_reasons?.map((r, i) => (
                      <div key={i} style={{ fontSize:11, color:'#f87171', marginBottom:4, display:'flex', gap:5 }}>
                        <span>✗</span><span>{r}</span>
                      </div>
                    ))}
                  </div>
            ...
            {rightTab === 'history' && (
              <div>
                ...
                {history.map(h => {
                  ...
                  return (
                    <div key={h.id} style={{
                      background:'#13192e', border:`1px solid ${cfg.border}22`,
                      ...
                      <div style={{ fontSize:11, color:'#64748b', marginTop:2 }}>
                        {fmtU(h.price)} · {new Date(h.ts+'Z').toLocaleString('es-MX', { month:'short', day:'numeric', hour:'2-digit', minute:'2-digit' })}
                      </div>
                    </div>
                  )
                })}
      ...
      {showAddPos && (
        <Modal title="Agregar Posición" onClose={() => setShowAddPos(false)}>
          <form onSubmit={handleAddPos}>
            <FSelect label="Activo" value={posForm.ticker} onChange={e=>setPosForm(f=>({...f,ticker:e.target.value}))}>
      ...
      {showAddAlert && (
        <Modal title="Nueva Alerta" onClose={() => setShowAddAlert(false)}>
          <form onSubmit={handleAddAlert}>
            <FSelect label="Activo" value={alertForm.ticker} onChange={e=>setAlertForm(f=>({...f,ticker:e.target.value}))}>
              {ALL_TICKERS.map(t=><option key={t} value={t}>{t}</option>)}
            </FSelect>
            <FSelect label="Tipo de alerta" value={alertForm.alert_type} onChange={e=>setAlertForm(f=>({...f,alert_type:e.target.value}))}>
              <option value="price">💰 Alerta de precio</option>
            </FSelect>
            {alertForm.alert_type === 'price' && (
              <>
                ...
                <FSelect label="Dirección" value={alertForm.direction} onChange={e=>setAlertForm(f=>({...f,direction:e.target.value}))}>
